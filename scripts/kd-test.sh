#!/usr/bin/env bash
#
# kd-test.sh -- smoke-test a kd-mcp streamable-http server end to end.
#
# Steps: open an MCP session -> status (pid) -> ensure attached -> break in
#        -> whereami -> resume the target.
#
# Usage:
#   ./kd-test.sh [HOST:PORT] [CONNECT_STRING]
# Examples:
#   ./kd-test.sh
#   ./kd-test.sh 192.168.56.11:8001 "net:port=55001,key=1.2.3.4"
...
echo; echo "### ensure attached (no-op if already connected)"
call kernel_attach "{\"connect_string\":\"$CONNECT_STRING\",\"timeout\":120}"

echo; echo "### break in (freeze the target so whereami has a context)"
call break_in '{"timeout":20}'

echo; echo "### whereami (rip + nearest symbol + top frames)"
call whereami
